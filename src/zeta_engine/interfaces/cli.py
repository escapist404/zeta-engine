...
from zeta_engine.infrastructure.constants import ALLOWED_DOMAINS, SEED_URLS
from zeta_engine.ingestion.crawler import crawl_urls
...
from zeta_engine.evaluation.runner import DEFAULT_BASE_URL, run_evaluation, run_rag_evaluation
...
from zeta_engine.interfaces.web import serve
...
def format_rag_debug(response: dict[str, object]) -> str:
    ...
    action_labels = {
        ...
        "fallback_without_evidence": "无证据降级回答",
        "invalid_response": "模型响应无效",
        "invalid_calculation": "计算请求无效",
        "collection_unavailable": "完整表格技能不可用",
        "duplicate_collection": "跳过重复集合扫描",
        "no_results": "没有检索结果",
    }
    status_labels = {
        "answered": "已回答",
    ...
    for index, item in enumerate(trace, start=1):
        ...
        if isinstance(queries, list) and queries:
            lines.append(
                "  调用技能: Hybrid 检索 · " + " | ".join(map(str, queries))
            )

        collection_query = item.get("collection_query")
        if isinstance(collection_query, str) and collection_query:
            lines.append(f"  调用技能: 完整表格扫描 · {collection_query}")
        ...
        for revision_name, label in (("final_revision", "最终修订"),):
